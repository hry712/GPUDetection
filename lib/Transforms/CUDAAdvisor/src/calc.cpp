...
double calcBD(void* trace, int& numBB_con, int& numBB_div)
{
	...
	for (long i=1; i<length; i++) { //TODO this is the bottleneck
...
double calcRD(void* itrace, double &avgrd, FILE* fout)
{
	...
	rd[1] = -1;
	int streamCnt = 0;
	for (i=1; i<length; i++) { //TODO this is the bottleneck
...
long calcMD(void* itrace, long *&memBins, FILE* outfile)
{
	...
	for (i=1; i<length; i++) {
		...
		if (searchL == cacheLines.end() ) { 	//didn't find this source line, totally new
