...
namespace{
    ...
    struct instru_host : public ModulePass{
        ...
        virtual bool runOnModule(Module &M) {
			...
			hookEA= cast<Function>(hookFuncEA);
        ...
        virtual bool runOnBasicBlock(Function::iterator &BB){
	    	...
	    	for(BasicBlock::iterator BI = BB->begin(), BE = BB->end(); BI != BE; ++BI){
				...
				if ( auto *op = dyn_cast<CallInst>( &(*BI) ) ){
					...
					if (loc) {
						int locLine =  loc.getLine();
						int locCol =  loc.getCol();
						builder.CreateCall(hookFCall, {ptr_caller, strPtr, builder.getInt32(locLine), builder.getInt32(locCol) } );
					}
    ...
    struct instru_kernel_memory : public ModulePass{
        ...
        virtual bool runOnModule(Module &M){
			...
			for(Module::iterator F = M.begin(), E = M.end(); F!= E; ++F){	// it is all about mangling
    ...
    struct instru_kernel_branch : public ModulePass{
        ...
        virtual bool runOnModule(Module &M){
			...
			VoidTy = Type::getVoidTy(C); 
			NamedMDNode* MDannotatios = M.getNamedMetadata("nvvm.annotations");
			for(unsigned i=0; i<MDannotatios->getNumOperands(); i++) { //auto detect GPU kernels
				MDNode* nn = MDannotatios->getOperand(i);
				for(unsigned j=0; j<nn->getNumOperands(); j++){
					if(! nn->getOperand(j))
						continue;
			...
			for(Module::iterator F = M.begin(), E = M.end(); F!= E; ++F){
				...
				if (!yeskernel){
					...
					if ( auto *op = dyn_cast<CallInst>(F->getEntryBlock().getFirstNonPHI())){	//we know that the first inst of cuda kernel is the InitKernel
						p_stackzone = (Value*) op;
						p_stackzone -> dump();
					}
				}
    ...
    struct instru_kernel_sig : public ModulePass{
        ...
        virtual bool runOnModule(Module &M){
			...
			for(Module::iterator F = M.begin(), E = M.end(); F!= E; ++F){	// round 1
				bool yeskernel = false;
				std::string fname = F->getName().str();
				for (int ii=0; ii<KNcnt; ii++)
			...
			for(Module::iterator F = M.begin(), E = M.end(); F!= E; ++F){	// round 2
			...
			for(Module::iterator F = M.begin(), E = M.end(); F!= E; ++F){	// round 3 
				bool oldkernel = false;
				bool newkernel = false;
				std::string fname = F->getName().str();
				int ii;
			...
			int TDcnt = 0;
			for(Module::iterator F = M.begin(), E = M.end(); F!= E; ++F){	// round 4
				std::string fname = F->getName().str();
				for (int ii=0; ii<KNcnt; ii++)
